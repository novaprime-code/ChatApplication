                    ...
                    <li v-for="message in messages" :key="message.id"  >
                        <div class="d-flex justify-content-start mb-4" v-if="message.sender_id == sender.id" >
                            <div class="img_cont_msg">
                                <img src="https://xsgames.co/randomusers/avatar.php?g=pixel" class="rounded-circle user_img_msg">
                            </div>
...
export default {
    ...
    methods:{
        ...
        getUserMessage(receiverId,senderId){
            axios.get('api/chat/messages',{ receiver : receiverId, sender : senderId })
            .then((res) => {
                if(res.status == 200){
                    this.messages = []
